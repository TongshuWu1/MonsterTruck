...
# Logging wrappers from config (fallback to plain print)
...
if "_show_replay" not in globals():
    def _show_replay(frames, fps=20):
        _logp("replay_messages", f"[EXACTGP_GLOBAL] replay disabled/fallback (frames={len(frames)}, fps={fps})")
...
_logp("method_setup", f"[{METHOD}] full-replay exact retraining (no cap) | init={int(X0.shape[0])} update_uses=all_memory | update_every={UPDATE_EVERY}")
...
_logp("method_setup", f"[{METHOD}] Training initial Exact GPR models on {X_fit_cur.shape[0]} samples...")
