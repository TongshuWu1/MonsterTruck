...
W_THDOT = 0.0
W_U = 0.1
# Extra per-step reward while the state is already inside the success/holding region.
# Implemented as a negative cost offset for planner/env consistency.
...
class CartPoleSwingUpTask(BaseTask):
    ...
    def _hold_mask_np(self, s):
        ...
        return (
            ...
            & (np.abs(x0) <= self.success_x_abs_max)
            & (np.abs(xdot) <= self.success_xdot_abs_max)
            & (np.abs(thdot) <= self.success_thdot_abs_max)
        )
    ...
    def stage_cost_tf(self, s, u):
        ...
        if self.hold_bonus_when_holding != 0.0:
            hold_mask = tf.logical_and(
                tf.logical_and(tf.cos(th) >= tf.constant(self.success_cos_th_min, DTF),
                               tf.abs(x) <= tf.constant(self.success_x_abs_max, DTF)),
                tf.logical_and(tf.abs(xdot) <= tf.constant(self.success_xdot_abs_max, DTF),
                               tf.abs(thdot) <= tf.constant(self.success_thdot_abs_max, DTF)),
            )
