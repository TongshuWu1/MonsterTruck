...
# Optional print gating from engine/config logging flags
def _eprint(*args, **kwargs):
    try:
        if bool(globals().get("LOG_EVAL_TABLES", True)):
...
def _save_combined_plots_png(out_dir: Path, pattern='*.png', out_name='00_all_plots_combined.png', cols=2, tile_pad=16, bg=(255,255,255)):
    """Create a single contact-sheet PNG containing all generated plot PNGs in out_dir."""
    pngs = sorted([p for p in out_dir.glob(pattern) if p.name != out_name])
    if not pngs:
        return None

    # Load and normalize to RGB; keep original images for legibility (no aggressive downscaling).
    imgs = []
    for p in pngs:
        try:
            ...
            _eprint(f'[combined-plots] skip {p.name}: {e}')
    if not imgs:
        return None
    ...
    # Use a uniform tile size based on max dimensions; add a small caption strip for filename.
    caption_h = 28
    tile_w = max(im.size[0] for _, im in imgs)
    tile_h = max(im.size[1] for _, im in imgs) + caption_h

    sheet_w = cols * tile_w + (cols + 1) * tile_pad
    sheet_h = rows * tile_h + (rows + 1) * tile_pad
    ...
    for idx, (name, im) in enumerate(imgs):
        ...
        # Letterbox each image into tile while preserving aspect ratio.
        boxed = ImageOps.contain(im, (tile_w, tile_h - caption_h), method=Image.Resampling.BICUBIC if hasattr(Image, 'Resampling') else Image.BICUBIC)
        px = x0 + (tile_w - boxed.size[0]) // 2
        py = y0
        sheet.paste(boxed, (px, py))

        draw = ImageDraw.Draw(sheet)
        caption = name
        # Keep caption simple and short if filename is long.
        if len(caption) > 56:
            caption = caption[:53] + '...'
        draw.text((x0 + 4, y0 + tile_h - caption_h + 6), caption, fill=(30,30,30))
...
def save_plots(reg, out_dir: Path):
    ...
    combined_path = _save_combined_plots_png(out_dir)
    if combined_path is not None:
        _eprint(f"Saved combined plot sheet: {combined_path}")
...
def main():
    ...
    _eprint(f'Loaded {n} worker artifacts')
