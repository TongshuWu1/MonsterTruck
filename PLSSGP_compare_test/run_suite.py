...
def _stream_worker(cmd, *, cwd=None, env=None, dashboard=None, echo_logs=True, echo_progress_lines=False):
    ...
    try:
        ...
        for raw_line in proc.stdout:
            ...
            if (not _is_frame_evt) and echo_logs:
                _looks_progress = (" it/s | " in line and " step " in line)
                if (not _looks_progress) or echo_progress_lines:
                    print(line)
...
def main() -> None:
    ...
    env_module = str(cfg.get('env_module', 'envs/mountaincar_hold/env.py'))
    ...
    for run_id in range(int(n_runs)):
        ...
        for method_name in methods_to_run:
            ...
            rc = _stream_worker(
                cmd, cwd=str(here), dashboard=dashboard,
                echo_logs=bool(cfg.get("suite_worker_stdout", True)),
                echo_progress_lines=bool(cfg.get("suite_progress_lines", False)),
            )
            if rc != 0:
