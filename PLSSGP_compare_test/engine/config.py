...
PROJECT_CONFIG = {
    ...
    "suite": {
        ...
        "env_module": "envs/mountaincar/env.py",  # fallback; suite_config.py prefers ENV_NAME mapping below
    ...
    "bootstrap": {
        # Initial offline random collection used to build X0/Y0 before any method training
        "initial_random_steps": 400,
    },
...
ENV_NAME = "mountaincar"   # choices: "cartpole_swingup", "mountaincar", "mountaincar_hold"
ENV_MODULE_BY_NAME = {
    "cartpole_swingup": "envs/cartpole_swingup/env.py",
    "mountaincar": "envs/mountaincar/env.py",
...
_BOOT = dict(PROJECT_CONFIG.get("bootstrap", {}))
...
SEED_BASE = int(_RT.get("seed_base", 0))  # default only; overridden per worker
N_RUNS = int(_RT.get("n_runs", 10))
N_EPISODES_PER_RUN = int(_RT.get("episodes_per_run", 2))
MAX_STEPS_PER_EP = int(_RT.get("max_steps_per_ep", 600))  # should match MAX_EPISODE_STEPS

# Initial offline random bootstrap dataset size (used in experiments.py before method training)
INIT_RANDOM_COLLECTION_STEPS = int(_BOOT.get("initial_random_steps", 400))
...
SUCCESS_HOLD_STEPS_BY_ENV = dict(_SUC.get("hold_steps_by_env", {"cartpole_swingup": 200, "mountaincar": 1, "mountaincar_hold": 200}))
