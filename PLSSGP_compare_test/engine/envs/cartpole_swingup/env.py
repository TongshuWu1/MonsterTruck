...
def cartpole_stage_reward(x, x_dot, theta, theta_dot, u):
    ...
    # Fallback-only bonus while inside the success region (kept small and tuneable by editing this literal).
    hold_bonus_when_holding = 0.05
    is_holding = (
        (math.cos(theta) >= 0.98)
        and (abs(x) <= 0.35)
        and (abs(x_dot) <= 1.0)
        and (abs(theta_dot) <= 2.5)
    )
    if hold_bonus_when_holding != 0.0 and is_holding:
        cost -= hold_bonus_when_holding
    return float(-cost)
