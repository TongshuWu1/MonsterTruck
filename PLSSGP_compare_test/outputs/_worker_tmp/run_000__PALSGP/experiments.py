...
ap.add_argument("--env-module", default="envs/mountaincar_hold/env.py")
...
_exec_file(here / "config.py", g)  # load config early so bootstrap knobs affect initial random collection

# Early logging fallbacks so functions defined before config.py can safely log
def _logp(cat, *args, **kwargs):
    return print(*args, **kwargs)

def _log_method(method, hold_steps=None):
    print(f"\n=== {method} ===" if hold_steps is None else f"\n=== {method} | hold={int(hold_steps)} ===")
...
def collect_random_transitions_rendered_cartpole(
    ...
    if len(frames) > 0:
        ...
        _logp("replay_messages", "⚠️ No frames collected (check frame_stride / resize).")
    ...
    _logp("data_bootstrap_summary", "Collected X0 shape:", X0.shape, " (6D features)")
    _logp("data_bootstrap_summary", "Targets shapes:", Ydx0.shape, Ydxdot0.shape, Ydth0.shape, Ydthdot0.shape)
    _logp("data_bootstrap_summary", f"Kept={traj['kept']}  Dropped(respawn)={traj['dropped_respawn']}  Resets={traj['resets']}")

    return X0, Ydx0, Ydxdot0, Ydth0, Ydthdot0, frames, traj


# ---- run it ----
SEED = int(globals().get("SEED_BASE", 0))
X0, Ydx0, Ydxdot0, Ydth0, Ydthdot0, frames0, traj0 = collect_random_transitions_rendered_cartpole(
    n_steps=int(globals().get("INIT_RANDOM_COLLECTION_STEPS", 400)),
    seed=SEED,
    max_episode_steps=500,
    start_down=True,
    frame_stride=2,
...
# Logging wrappers (provided by engine/config.py; fall back to plain print if absent)
_lp = globals().get("log_print", None)
_lmb = globals().get("log_method_banner", None)
def _logp(cat, *args, **kwargs):
    return (_lp(cat, *args, **kwargs) if callable(_lp) else print(*args, **kwargs))
def _log_method(method, hold_steps=None):
    if callable(_lmb):
        return _lmb(method, hold_steps)
    print(f"\n=== {method} ===" if hold_steps is None else f"\n=== {method} | hold={int(hold_steps)} ===")
...
def _show_replay(frames, fps=20):
    if len(frames) == 0:
        _logp("replay_messages", "⚠️ No replay frames.")
        return
    fig, ax = plt.subplots(figsize=(8, 5))
    im = ax.imshow(frames[0])
    ax.axis("off")
...
# MAIN: PALSGP
# ============================================================
if _method_enabled("PALSGP"):
    METHOD = "PALSGP"
    _log_method(METHOD, SUCCESS_HOLD_STEPS)
    ...
    _logp("method_setup", "✅ Z_GLOBAL init:", Z_GLOBAL.shape, "anchors:", anchor_idx.shape)

    # initial global training
    _logp("method_setup", "Training initial global OSGPR models...")
    ...
    for run in range(N_RUNS):
        ...
        for ep in range(N_EPISODES_PER_RUN):
            ...
            for step in range(MAX_STEPS_PER_EP):
                ...
                if ((t_global + 1) % int(UPDATE_EVERY) == 0) and (len(Xbuf) > 0):
                    ...
                    (_logp("update_events", f"{METHOD} global update | r{run} e{ep} t={t_global} | pred_time={pred_dt:.3f}s train_time={trn_dt:.3f}s") if "_logp" in globals() else print(f"{METHOD} global update | r{run} e{ep} t={t_global} | pred_time={pred_dt:.3f}s train_time={trn_dt:.3f}s"))
                    models = (m_dx, m_dxdot, m_dth, m_dthdot)
            ...
            (_logp("episode_summary", f"[{METHOD}] episode | r{run} e{ep} | reward={float(ep_reward):.2f} steps={int(ep_steps)} hold_max={int(hold_max)} success={bool(ep_hold_complete_t is not None)} first_success_t={int(ep_hold_complete_t) if ep_hold_complete_t is not None else None} ep_wall_excl_vis={_ep_wall_excl_vis:.3f}s cum_wall_excl_vis={_cum_wall_excl_vis:.3f}s decision_mean={_ep_pred_mean:.6f}s") if "_logp" in globals() else print(f"[{METHOD}] episode | r{run} e{ep} | reward={float(ep_reward):.2f} steps={int(ep_steps)} hold_max={int(hold_max)} success={bool(ep_hold_complete_t is not None)} first_success_t={int(ep_hold_complete_t) if ep_hold_complete_t is not None else None} ep_wall_excl_vis={_ep_wall_excl_vis:.3f}s cum_wall_excl_vis={_cum_wall_excl_vis:.3f}s decision_mean={_ep_pred_mean:.6f}s"))
        ...
        _logp("run_summary", f"[{METHOD}] run {run+1}/{N_RUNS}: reward={total_reward_run:.2f} success={run_success} first_success_t={first_success_t}")

        if CAPTURE_REPLAY:
            _logp("replay_messages", f"\n▶️ Replay: {METHOD} RUN {run} (all episodes) | frames={len(run_frames)} | fps={REPLAY_FPS}")
            _show_replay(run_frames, fps=REPLAY_FPS)

    _logp("run_summary", f"\n✅ {METHOD} finished. Registry runs = {len(EVAL_REGISTRY[METHOD]['run_stats'])}")

else:
    print("\n⏭️ Skipping PALSGP (ONLY_METHOD=" + str(_ONLY_METHOD) + ")")
# ===========================
...
# SVGP builders
def make_svgp(Z, kernel, noise=NOISE_INIT, train_hypers=False, train_inducing=False):
    Z = np.asarray(Z, dtype=np.float64)
    ...
    gpflow.set_trainable(m.inducing_variable, bool(train_inducing))
    if not train_hypers:
        try:
            m.kernel.variance.trainable = False
            m.kernel.lengthscales.trainable = False
...
# Run SVGP_GLOBAL
if _method_enabled("SVGP_GLOBAL"):
    METHOD = "SVGP_GLOBAL"
    _log_method(METHOD, SUCCESS_HOLD_STEPS)

    SVGP_TRAIN_HYPERS_LOCAL = bool(globals().get("SVGP_TRAIN_HYPERS", True))
    SVGP_TRAIN_INDUCING_LOCAL = bool(globals().get("SVGP_TRAIN_INDUCING", False))
    SVGP_STEPS_INIT_LOCAL = int(globals().get("SVGP_STEPS_INIT", 250))
    SVGP_STEPS_PER_UPDATE_LOCAL = int(globals().get("SVGP_STEPS_PER_UPDATE", 80))
    SVGP_LR_LOCAL = float(globals().get("SVGP_LR", 0.02))
    SVGP_REPLAY_TRAIN_MAX_LOCAL = int(globals().get("SVGP_REPLAY_TRAIN_MAX", 2500))
    SVGP_REFIT_GLOBAL_EACH_EP_LOCAL = bool(globals().get("SVGP_REFIT_GLOBAL_EACH_EP", True))
    SVGP_REFIT_REPLAY_MAX_LOCAL = int(globals().get("SVGP_REFIT_REPLAY_MAX", 4000))
    SVGP_REFIT_STEPS_LOCAL = int(globals().get("SVGP_REFIT_STEPS", 250))
    SVGP_REFIT_LR_LOCAL = float(globals().get("SVGP_REFIT_LR", SVGP_LR_LOCAL))

    rng0 = np.random.default_rng(SEED_BASE)
    M_GLOBAL = int(min(M_GLOBAL_INIT, X0.shape[0]))
    Z_GLOBAL = np.asarray(X0[rng0.choice(X0.shape[0], size=M_GLOBAL, replace=False)], dtype=np.float64)

    m_dx     = make_svgp(Z_GLOBAL, k_dx,     noise=NOISE_INIT, train_hypers=SVGP_TRAIN_HYPERS_LOCAL, train_inducing=SVGP_TRAIN_INDUCING_LOCAL)
    m_dxdot  = make_svgp(Z_GLOBAL, k_dxdot,  noise=NOISE_INIT, train_hypers=SVGP_TRAIN_HYPERS_LOCAL, train_inducing=SVGP_TRAIN_INDUCING_LOCAL)
    m_dth    = make_svgp(Z_GLOBAL, k_dth,    noise=NOISE_INIT, train_hypers=SVGP_TRAIN_HYPERS_LOCAL, train_inducing=SVGP_TRAIN_INDUCING_LOCAL)
    m_dthdot = make_svgp(Z_GLOBAL, k_dthdot, noise=NOISE_INIT, train_hypers=SVGP_TRAIN_HYPERS_LOCAL, train_inducing=SVGP_TRAIN_INDUCING_LOCAL)

    _logp("method_setup", f"SVGP baseline mode: train_hypers={SVGP_TRAIN_HYPERS_LOCAL} train_inducing={SVGP_TRAIN_INDUCING_LOCAL} refit_ep={SVGP_REFIT_GLOBAL_EACH_EP_LOCAL} steps_init={SVGP_STEPS_INIT_LOCAL} steps_upd={SVGP_STEPS_PER_UPDATE_LOCAL}")
    _logp("method_setup", "Initial SVGP training...")
    _ = svgp_train_steps(m_dx,     X0, Ydx0,     steps=SVGP_STEPS_INIT_LOCAL, lr=SVGP_LR_LOCAL)
    _ = svgp_train_steps(m_dxdot,  X0, Ydxdot0,  steps=SVGP_STEPS_INIT_LOCAL, lr=SVGP_LR_LOCAL)
    _ = svgp_train_steps(m_dth,    X0, Ydth0,    steps=SVGP_STEPS_INIT_LOCAL, lr=SVGP_LR_LOCAL)
    _ = svgp_train_steps(m_dthdot, X0, Ydthdot0, steps=SVGP_STEPS_INIT_LOCAL, lr=SVGP_LR_LOCAL)
        ...
        for ep in range(N_EPISODES_PER_RUN):
            ...
            for step in range(MAX_STEPS_PER_EP):
                ...
                if ((t_global + 1) % int(UPDATE_EVERY) == 0) and (len(Xmem) > 200):
                    did_update = True
                    nmem = len(Xmem)
                    bsz = int(min(SVGP_REPLAY_TRAIN_MAX_LOCAL, nmem))
                    ...
                    tu0 = time.perf_counter()
                    _ = svgp_train_steps(m_dx,     Xb, ydx_b,     steps=SVGP_STEPS_PER_UPDATE_LOCAL, lr=SVGP_LR_LOCAL)
                    _ = svgp_train_steps(m_dxdot,  Xb, ydxdot_b,  steps=SVGP_STEPS_PER_UPDATE_LOCAL, lr=SVGP_LR_LOCAL)
                    _ = svgp_train_steps(m_dth,    Xb, ydth_b,    steps=SVGP_STEPS_PER_UPDATE_LOCAL, lr=SVGP_LR_LOCAL)
                    _ = svgp_train_steps(m_dthdot, Xb, ydthdot_b, steps=SVGP_STEPS_PER_UPDATE_LOCAL, lr=SVGP_LR_LOCAL)
                    trn_dt = float(time.perf_counter() - tu0)
            ...
            # Optional: episode-end global-Z refit + reinit (fairer/closer to OSGPR/PALSGP global maintenance)
            if SVGP_REFIT_GLOBAL_EACH_EP_LOCAL and (len(Xmem) >= max(int(M_GLOBAL), 32)):
                if len(Xmem) > SVGP_REFIT_REPLAY_MAX_LOCAL:
                    idxr = rng.choice(len(Xmem), size=SVGP_REFIT_REPLAY_MAX_LOCAL, replace=False)
                else:
                    idxr = np.arange(len(Xmem))
                ...
                _ = svgp_train_steps(m_dx,     Xr, ydx_r,     steps=SVGP_REFIT_STEPS_LOCAL, lr=SVGP_REFIT_LR_LOCAL)
                _ = svgp_train_steps(m_dxdot,  Xr, ydxdot_r,  steps=SVGP_REFIT_STEPS_LOCAL, lr=SVGP_REFIT_LR_LOCAL)
                _ = svgp_train_steps(m_dth,    Xr, ydth_r,    steps=SVGP_REFIT_STEPS_LOCAL, lr=SVGP_REFIT_LR_LOCAL)
                _ = svgp_train_steps(m_dthdot, Xr, ydthdot_r, steps=SVGP_REFIT_STEPS_LOCAL, lr=SVGP_REFIT_LR_LOCAL)
                models = (m_dx, m_dxdot, m_dth, m_dthdot)
                pack_tensors = packs_from_global(models, Z_GLOBAL)
    ...
    print("\n⏭️ Skipping SVGP_GLOBAL (ONLY_METHOD=" + str(_ONLY_METHOD) + ")")
# ===========================
...
# Run OSGPR_GLOBAL
if _method_enabled("OSGPR_GLOBAL"):
    METHOD = "OSGPR_GLOBAL"
    _log_method(METHOD, SUCCESS_HOLD_STEPS)
    ...
    _logp("method_setup", "Training initial OSGPR models...")
    m_dx,     _ = init_osgpr_from_prior(X0, Ydx0,     Z_GLOBAL, k_dx,     noise=NOISE_INIT, iters=650, lr=0.02)
    m_dxdot,  _ = init_osgpr_from_prior(X0, Ydxdot0,  Z_GLOBAL, k_dxdot,  noise=NOISE_INIT, iters=650, lr=0.02)
    m_dth,    _ = init_osgpr_from_prior(X0, Ydth0,    Z_GLOBAL, k_dth,    noise=NOISE_INIT, iters=650, lr=0.02)
    m_dthdot, _ = init_osgpr_from_prior(X0, Ydthdot0, Z_GLOBAL, k_dthdot, noise=NOISE_INIT, iters=650, lr=0.02)
    ...
    print("\n⏭️ Skipping OSGPR_GLOBAL (ONLY_METHOD=" + str(_ONLY_METHOD) + ")")
# ===========================
