...
def load_env_module(globals_dict, module_path="envs/mountaincar_hold/env.py"):
    code = Path(module_path).read_text(encoding="utf-8")
    exec(compile(code, module_path, "exec"), globals_dict, globals_dict)



def load_task_env_into_globals(globals_dict, env_name="mountaincar_hold"):
    ...
    if str(env_name).lower() in ("mountaincar", "mountaincar_hold", "mountaincar_continuous_hold", "continuous_mountaincar", "continuous_mountaincar_hold", "mountain_car"):
