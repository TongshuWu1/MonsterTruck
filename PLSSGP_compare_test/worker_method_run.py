...
FRAME_EVENT_PREFIX = "__LIVE_FRAME__ "
...
def _replace_or_raise(text: str, old: str, new: str, *, label: str) -> str:
    if old not in text:
        raise RuntimeError(f"patch_experiments_text: expected pattern not found for {label}")
    return text.replace(old, new, 1)


def _assert_contains(text: str, needle: str, *, label: str) -> None:
    if needle not in text:
        raise RuntimeError(f"patch_experiments_text: expected marker missing after patch ({label})")
...
def patch_experiments_text(text: str, only_method: str = "") -> str:
    ...
    text = _replace_or_raise(
        ...
        label='global update timing print'
    )
    ...
    text = _replace_or_raise(text, ep_marker, ep_repl, label='episode summary print injection')
    ...
    _assert_contains(text, 'PROGRESS_EVERY_STEPS', label='progress cadence injected')
    _assert_contains(text, 'global update | r{run} e{ep} t={t_global}', label='global update print injected')
    _assert_contains(text, '[{METHOD}] episode | r{run} e{ep}', label='episode summary print injected')
    return text
...
class PygameLiveViewer:
    def __init__(self, enabled=True, every_steps=50, min_dt=0.03, size=(1600, 980), caption='GP Comparison Live'):
    ...
    def on_log_line(self, line):
        s = str(line)
        if s.startswith(FRAME_EVENT_PREFIX):
            self._ingest_frame_event(s)
    ...
    def _ingest_frame_event(self, line):
        try:
            import json, base64, io
            from PIL import Image
            import numpy as np
            payload = json.loads(str(line)[len(FRAME_EVENT_PREFIX):])
            b64 = payload.get('b64', '')
            if not b64:
                return
            raw = base64.b64decode(b64.encode('ascii'))
            img = Image.open(io.BytesIO(raw)).convert('RGB')
            self._frame_rgb = np.asarray(img, dtype=np.uint8)
            ov = payload.get('overlay')
            if ov:
                self.latest_overlay = str(ov)
        except Exception:
            return
    ...
    def _draw_method_panels(self):
        ...
        x0 = 930
        y = 10
        y = self._draw_text(x0, y, 'GP Comparison Dashboard', color=(255,255,180), font=self._font_big)
        cur = self.current
    ...
    def maybe_update(self, step_i, x, theta, text=''):
        ...
        # Accept environment-specific renderer hooks when available; keep dashboard usable even without a frame renderer.
        main_mod = sys.modules.get('__main__')
        fn = None
        for name in ('render_state_frame_from_state', 'render_live_frame_from_state', 'render_cartpole_frame_from_state'):
            cand = getattr(main_mod, name, None)
            if callable(cand):
                fn = cand
                break
...
class StdoutEventViewer:
    """Headless viewer used by run_suite.py unified dashboard mode.

    It emits the live overlay text to stdout so the parent process can parse it and update a single
    unified pygame dashboard across methods, while keeping workers headless.
    """
    def __init__(self, enabled=True, every_steps=50, min_dt=0.03):
        self.enabled = bool(enabled)
        self.every_steps = max(1, int(every_steps))
        self.min_dt = float(min_dt)
        self._last_step = -10**9
    ...
    def maybe_update(self, step_i, x, theta, text=''):
        ...
        if (int(step_i) - self._last_step) < self.every_steps and (now - self._last_t) < self.min_dt:
            return
        if text:
            # Parent suite dashboard parser expects the same raw line format used by local pygame viewer overlays.
            print(str(text), flush=True)

        # Emit a compressed frame event so the parent unified dashboard can display live rendering.
        try:
            import sys as _sys, io, json, base64
            import numpy as np
            from PIL import Image
            ...
            if fn is not None:
                frame = None
                try:
                    frame = fn(float(x), float(theta), x_threshold=2.4, W=900, H=540)
                except TypeError:
                    try:
                        frame = fn(float(x), float(theta), W=900, H=540)
                    except TypeError:
                        frame = fn(float(x), float(theta))
                if frame is not None:
                    ...
                    if arr.ndim == 3 and arr.shape[2] >= 3:
                        ...
                        payload = {
                            ...
                            'h': int(arr.shape[0]),
                            'step': int(step_i),
                            'overlay': ('' if text is None else str(text)),
                            'b64': base64.b64encode(bio.getvalue()).decode('ascii'),
                        }
                        print(FRAME_EVENT_PREFIX + json.dumps(payload, separators=(',', ':')), flush=True)
        except Exception:
            pass
...
def main() -> None:
    ...
    ap.add_argument('--viewer-mode', default='local', choices=['local', 'headless', 'none'])
    args = ap.parse_args()
    ...
    try:
        ...
        _viewer_mode = str(args.viewer_mode).strip().lower()
        if _viewer_mode == 'local':
            init_globals['LIVE_VIEWER'] = PygameLiveViewer(
                enabled=_parse_bool(args.live_render),
                every_steps=int(args.live_every_steps),
                size=(1600, 980),
                caption=f"GP Compare | run {args.run_id} | {str(args.only_method).strip() or 'ALL'}"
            )
        elif _viewer_mode == 'headless':
            init_globals['LIVE_VIEWER'] = StdoutEventViewer(
                enabled=_parse_bool(args.live_render),
                every_steps=int(args.live_every_steps),
            )
        else:
            init_globals['LIVE_VIEWER'] = StdoutEventViewer(enabled=False)
